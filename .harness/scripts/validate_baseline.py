...
def sqlite_quick_check(sqlite_path: Path) -> str:
    con = sqlite3.connect(str(sqlite_path))
    try:
...
def main() -> int:
    ...
    parser.add_argument("--sqlite", required=True, help="Path to the full metadata SQLite DB")
    parser.add_argument("--analysis-ready", help="Optional path to the analysis-ready SQLite DB")
    ...
    media_assets_count = int(table_counts.get("media_assets", -1))
    channel_count = int(table_counts.get("instrumentation_channels", -1))
    injury_count = int(table_counts.get("injury_metrics", -1))
    intrusions_count = int(table_counts.get("intrusion_measurements", -1))

    if media_assets_count == 0:
        blockers.append("SQLite media_assets table has 0 rows; data package/report link inventory is not loaded.")
    if channel_count == 0:
    ...
    if intrusions_count == 0:
        warnings.append("SQLite intrusion_measurements table has 0 rows; intrusion validation is outside current baseline.")
    ...
    if "tests" in table_counts:
        sqlite_side_pole_count = sqlite_count(
            ...
            SELECT COUNT(*)
            FROM tests t
            JOIN barrier_load_cell_classification c ON c.test_id = t.id
            WHERE t.test_configuration_key='VTP'
              AND c.family='side_pole_load_cell_barrier'
              AND c.classification_id='side_pole_load_cell_8'
            """,
        )
        checks.append({
    ...
    analysis_ready_path = Path(args.analysis_ready) if args.analysis_ready else None
    if analysis_ready_path:
        analysis_counts = sqlite_table_counts(analysis_ready_path)
        missing_analysis_tables = [
            table for table in ANALYSIS_READY_EXPECTED_COUNTS if table not in analysis_counts
        ]
        count_mismatches = {
            table: {
                "expected": expected,
                "observed": analysis_counts.get(table),
            }
            for table, expected in ANALYSIS_READY_EXPECTED_COUNTS.items()
            if analysis_counts.get(table) != expected
        }
        quick_check = sqlite_quick_check(analysis_ready_path)
        checks.append({
            "id": "analysis_ready_expected_counts",
            "status": "pass" if not missing_analysis_tables and not count_mismatches and quick_check == "ok" else "fail",
            "analysis_ready_path": str(analysis_ready_path),
            "missing_tables": missing_analysis_tables,
